...
class Encoder_baseline(nn.Module):
    ...
    def __init__(self, vectors, enc_hidden_dim, dec_hidden_dim, pad_idx, dropout, device):
        ...
        self.ctx_rnn = nn.GRU(self.emb_dim, enc_hidden_dim, batch_first=True, bidirectional=True)
    ...
    def forward(self, context_ids, answer_ids, answ_start, answ_end, ctx_lengths, answ_lenghts):
        ...
        ctx_last = torch.cat((ctx_hidden[-2,:,:],ctx_hidden[-1,:,:]), dim=1)
        # [bs, enc_hidden_dim]

        answ_embeds = self.emb_layer(answer_ids)
        # [bs, answ_len, emb_dim]

        packed_answ_ctx = pack_padded_sequence(answ_embeds, answ_lenghts.cpu(), batch_first=True, enforce_sorted=False)
        ...
        hidden = torch.tanh(self.to_dec(torch.cat((answ_last,ctx_last), dim=1)))
        # [bs, dec_hidden_dim]

        return ctx_outputs, hidden.unsqueeze(0)
...
class Attention_baseline(nn.Module):
    ...
    def forward(self, prev_hidden, enc_states, att_mask):

        #enc_states = [bs, ctx_len, enc_hidden_dim*2]
        #prev_hidden = [1, bs, dec_hidden_dim]
        #att_mask = [bs, ctx_len]

        prev_hidden = prev_hidden.permute(1,0,2).repeat(1,enc_states.shape[1],1)
        # [bs, ctx_len, dec_hidden_dim]

        combined = torch.cat((prev_hidden, enc_states), dim=2)
